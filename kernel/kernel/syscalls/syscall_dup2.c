...
long syscall_dup2(long oldfd, long newfd) {
  ...
  vfs_file_t* file = proc_fd_get(g_kernel.current_process, oldfd);
  if (file == NULL) { return -EBADF; }

  if (oldfd == newfd) { return newfd; }

  if (proc_fd_get(g_kernel.current_process, newfd) != NULL) {
    vfs_status_t status = syscall_close_fd(newfd);
    if (status != VFS_STATUS_OK) { return -vfs_status_to_errno(status); }
  }

  vfs_file_borrow(file);
  proc_fd_set(g_kernel.current_process, newfd, file);
  return newfd;
}
